...
@interface GANPieChart()<CAAnimationDelegate>
...
- (GANPieChartDataItem *)dataItemForIndex:(NSUInteger)index;
...
- (void)loadDefault{
    ...
    [_items enumerateObjectsUsingBlock:^(GANPieChartDataItem *item, NSUInteger idx, BOOL *stop) {
        if (total == 0){
...
- (void)recompute {
    ...
    self.innerCircleRadius = 0; // minimal / 6;
}
...
- (void)strokeChart{
    ...
    GANPieChartDataItem *currentItem;
    ...
    for (int i = 0; i < _items.count; i++) {
        ...
        // Draw Border
        float alpha = M_PI_2 * 4 * startPercentage - M_PI_2;
        CGPoint pointTo = CGPointMake(pointCenter.x + radius * 2 * cos(alpha), pointCenter.y + radius * 2 * sin(alpha));
        CAShapeLayer *line = [CAShapeLayer layer];
        UIBezierPath *linePath = [UIBezierPath bezierPath];
        [linePath moveToPoint: pointCenter];
        [linePath addLineToPoint:pointTo];
        line.path = linePath.CGPath;
        line.fillColor = nil;
        line.opacity = 1.0;
        line.strokeColor = _outlineColor.CGColor;
        [_pieLayer addSublayer:line];
    }
    
    // Circle
    CAShapeLayer *circle = [CAShapeLayer layer];
    
    UIBezierPath *path = [UIBezierPath bezierPathWithArcCenter:pointCenter
                                                        radius:radius * 2
                                                    startAngle:-M_PI_2
                                                      endAngle:M_PI_2 * 3
                                                     clockwise:YES];
    ...
    circle.strokeColor = _outlineColor.CGColor;
    circle.lineWidth   = 1;
    circle.path        = path.CGPath;
    [_pieLayer addSublayer:circle];
...
- (UILabel *)descriptionLabelForItemAtIndex:(NSUInteger)index{
    GANPieChartDataItem *currentDataItem = [self dataItemForIndex:index];
...
- (void)didTouchAt:(CGPoint)touchLocation
{
    ...
    if (self.shouldHighlightSectorOnTouch)
    {
        ...
        GANPieChartDataItem *currentItem = [self dataItemForIndex:index];
